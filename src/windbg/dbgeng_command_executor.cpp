...
class OutputCaptureCallbacks : public IDebugOutputCallbacks {
public:
  OutputCaptureCallbacks(const CommandExecutionOptions& options = {})
      : ref_count_(1), options_(options), line_count_(0), truncated_(false) {}

  STDMETHOD(QueryInterface)(REFIID riid, void** ppvObject) override {
    if (ppvObject == nullptr) {
      return E_POINTER;
    }
    if (riid == __uuidof(IUnknown) || riid == __uuidof(IDebugOutputCallbacks)) {
      *ppvObject = static_cast<IDebugOutputCallbacks*>(this);
      AddRef();
      return S_OK;
    }
    *ppvObject = nullptr;
  ...
  STDMETHOD_(ULONG, AddRef)() override { return static_cast<ULONG>(InterlockedIncrement(&ref_count_)); }
  ...
  STDMETHOD(Output)(ULONG mask, PCSTR text) override {
    ...
    while (start < input.size()) {
      ...
      std::string line = (end == std::string::npos) ? input.substr(start) : input.substr(start, end - start + 1);
      ...
      if (include_line) {
        output_ += line;
        if (end != std::string::npos || (line.size() > 0 && line.back() == '\n')) {
          line_count_++;
          if (options_.max_lines > 0 && line_count_ >= options_.max_lines) {
            output_ +=
                "\n... [Output truncated after reaching max_lines limit: " + std::to_string(options_.max_lines) + "]\n";
            truncated_ = true;
            break;
          }
        }
...
std::string HResultToString(HRESULT hr) {
  ...
  const DWORD size = FormatMessageA(
      ...
      static_cast<DWORD>(hr), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), reinterpret_cast<LPSTR>(&raw), 0, nullptr);
  ...
  for (int shift = 28; shift >= 0; shift -= 4) {
    hex.push_back(kHex[(hr >> shift) & 0x0F]);
  }
...
HRESULT EvaluateExtendedExpressionSafe(IDebugHostEvaluator2* evaluator, const wchar_t* expression,
                                       IModelObject** result) {
  __try {
    return evaluator->EvaluateExtendedExpression(USE_CURRENT_HOST_CONTEXT, expression, nullptr, result, nullptr);
...
CommandExecutionResult DbgEngCommandExecutor::Execute(const std::string& command,
  ...
  return DispatchToWorker([this, command, options]() { return ExecuteSynchronously(command, options); }, true);
}
...
CommandExecutionResult DbgEngCommandExecutor::DispatchToWorker(TaskFunction func, bool check_ready) {
  ...
  auto wrapped_task = [this, func = std::move(func), check_ready]() -> CommandExecutionResult {
    if (check_ready) {
...

  {
    std::lock_guard<std::mutex> lock(queue_mutex_);
    if (shutdown_) {
      return {false, "", "Executor is shutting down"};
    }
    task_queue_.push(ExecutionTask{std::move(wrapped_task), std::move(promise), true});
  }
  cv_.notify_one();
  return future.get();
}
...
void DbgEngCommandExecutor::WorkerThreadProc() {
  ...
  if (SUCCEEDED(DebugCreate(__uuidof(IDebugClient), reinterpret_cast<void**>(worker_client_.GetAddressOf())))) {
    (void)worker_client_.As(&worker_control_);
    (void)worker_client_.As(&interrupt_control_);
    (void)worker_client_.As(&worker_data_);
    (void)worker_client_.As(&worker_symbols_);
    (void)worker_client_.As(&worker_registers_);
    (void)worker_client_.As(&worker_systems_);
  }
  ...
  worker_systems_.Reset();
  worker_registers_.Reset();
  worker_symbols_.Reset();
  worker_data_.Reset();
  interrupt_control_.Reset();
  worker_control_.Reset();
  worker_client_.Reset();

  CoUninitialize();
}

CommandExecutionResult DbgEngCommandExecutor::ExecuteSynchronously(const std::string& command,
                                                                   const CommandExecutionOptions& options) {
  if (command.empty()) {
  ...
  if (client == nullptr || control == nullptr) {
    HRESULT hr = DebugCreate(__uuidof(IDebugClient), reinterpret_cast<void**>(client.GetAddressOf()));
    if (FAILED(hr)) {
      return {false, "", "DebugCreate failed: " + HResultToString(hr)};
    }
    hr = client.As(&control);
    if (FAILED(hr)) {
      return {false, "", "IDebugControl not available: " + HResultToString(hr)};
    }
  }
  ...
  HRESULT hr = client->SetOutputCallbacks(capture);
  if (FAILED(hr)) {
    capture->Release();
    return {
        false,
...
DebuggerExecutionState DbgEngCommandExecutor::GetExecutionState() {
  ...
  if (interrupt_control_ != nullptr) {
    ULONG raw_status = 0;
    if (SUCCEEDED(interrupt_control_->GetExecutionStatus(&raw_status))) {
      return ParseRawStatus(raw_status);
    }
  }
  return {};
...
CommandExecutionResult DbgEngCommandExecutor::EvaluateModel(const std::string& expression, int max_depth) {
  return DispatchToWorker([this, expression, max_depth]() { return EvaluateModelSynchronously(expression, max_depth); },
                          true);
}
...
CommandExecutionResult DbgEngCommandExecutor::ReadMemory(std::uint64_t address, std::uint32_t length) {
  return DispatchToWorker([this, address, length]() { return ReadMemorySynchronously(address, length); }, true);
}

CommandExecutionResult DbgEngCommandExecutor::ReadMemorySynchronously(std::uint64_t address, std::uint32_t length) {
  Microsoft::WRL::ComPtr<IDebugDataSpaces> data = worker_data_;
  if (data == nullptr) {
...
CommandExecutionResult DbgEngCommandExecutor::CarvePE(std::uint64_t address, std::uint32_t length) {
  return DispatchToWorker([this, address, length]() { return CarvePESynchronously(address, length); }, true);
}
...
CommandExecutionResult DbgEngCommandExecutor::CarvePESynchronously(std::uint64_t address, std::uint32_t length) {
  ...
  if (mem_data[pe_offset] != 'P' || mem_data[pe_offset + 1] != 'E' || mem_data[pe_offset + 2] != 0 ||
      mem_data[pe_offset + 3] != 0) {
...
CommandExecutionResult DbgEngCommandExecutor::WriteMemory(std::uint64_t address, const std::string& hex_data) {
  return DispatchToWorker([this, address, hex_data]() { return WriteMemorySynchronously(address, hex_data); }, true);
}

CommandExecutionResult DbgEngCommandExecutor::WriteMemorySynchronously(std::uint64_t address,
...
CommandExecutionResult DbgEngCommandExecutor::SearchMemory(std::uint64_t start_address, std::uint64_t end_address,
                                                           const std::string& pattern) {
  return DispatchToWorker([this, start_address, end_address,
                           pattern]() { return SearchMemorySynchronously(start_address, end_address, pattern); },
                          true);
}

CommandExecutionResult DbgEngCommandExecutor::SearchMemorySynchronously(std::uint64_t start_address,
  ...
  while (current < end_address) {
    if (SUCCEEDED(data->SearchVirtual(current, end_address - current, pattern_bytes.data(), (ULONG)pattern_bytes.size(),
                                      1, &found_addr))) {
...
CommandExecutionResult DbgEngCommandExecutor::GetModules() {
  return DispatchToWorker([this]() { return GetModulesSynchronously(); }, true);
}
      ...
      if (SUCCEEDED(symbols->GetModuleParameters(1, &base, 0, &params))) {
        ...
        switch (params.SymbolType) {
          case DEBUG_SYMTYPE_NONE:
            writer.StringValue("None");
            break;
          case DEBUG_SYMTYPE_COFF:
            writer.StringValue("COFF");
            break;
          case DEBUG_SYMTYPE_CODEVIEW:
            writer.StringValue("CodeView");
            break;
          case DEBUG_SYMTYPE_PDB:
            writer.StringValue("PDB");
            break;
          case DEBUG_SYMTYPE_EXPORT:
            writer.StringValue("Export");
            break;
          case DEBUG_SYMTYPE_DEFERRED:
            writer.StringValue("Deferred");
            break;
          case DEBUG_SYMTYPE_SYM:
            writer.StringValue("SYM");
            break;
          case DEBUG_SYMTYPE_DIA:
            writer.StringValue("DIA");
            break;
          default:
            writer.StringValue("Other");
            break;
        }
        writer.EndObject();
      }
...
CommandExecutionResult DbgEngCommandExecutor::GetBreakpoints() {
  return DispatchToWorker([this]() { return GetBreakpointsSynchronously(); }, true);
}
...
CommandExecutionResult DbgEngCommandExecutor::GetBreakpointsSynchronously() {
  ...
  if (control == nullptr) {
    ...
    client.As(&symbols);
  } else if (worker_client_ != nullptr) {
...
CommandExecutionResult DbgEngCommandExecutor::Disassemble(std::uint64_t address, std::uint32_t count) {
  return DispatchToWorker([this, address, count]() { return DisassembleSynchronously(address, count); }, true);
}
...
CommandExecutionResult DbgEngCommandExecutor::ReadString(std::uint64_t address, std::uint32_t max_length, bool wide) {
  return DispatchToWorker(
      [this, address, max_length, wide]() { return ReadStringSynchronously(address, max_length, wide); }, true);
}

CommandExecutionResult DbgEngCommandExecutor::ReadStringSynchronously(std::uint64_t address, std::uint32_t max_length,
                                                                      bool wide) {
  Microsoft::WRL::ComPtr<IDebugDataSpaces> data = worker_data_;
  if (data == nullptr) {
    Microsoft::WRL::ComPtr<IDebugClient> client;
  ...
  if (wide) {
    ...
    int u8_len = WideCharToMultiByte(CP_UTF8, 0, buf.data(), static_cast<int>(len), utf8.data(),
...
CommandExecutionResult DbgEngCommandExecutor::SetBreakpoint(const std::string& expression) {
  return DispatchToWorker([this, expression]() { return ExecuteSynchronously("bp " + expression, {}); }, true);
}

CommandExecutionResult DbgEngCommandExecutor::GetThreads() {
  return DispatchToWorker([this]() { return GetThreadsSynchronously(); }, true);
}

CommandExecutionResult DbgEngCommandExecutor::GetThreadsSynchronously() {
  Microsoft::WRL::ComPtr<IDebugSystemObjects> systems = worker_systems_;
  if (systems == nullptr) {
    Microsoft::WRL::ComPtr<IDebugClient> client;
    if (FAILED(DebugCreate(__uuidof(IDebugClient), reinterpret_cast<void**>(client.GetAddressOf())))) {
      return {false, "", "DebugCreate failed"};
