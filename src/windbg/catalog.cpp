...
const std::vector<CatalogEntry>& Catalog::GetEntries() {
  static const std::vector<CatalogEntry> entries = {
      ...
      {"bl_list_breakpoints",
       ...
       "bl",
       "Description:\n"
       " Displays a table of all current breakpoints. Includes breakpoint ID, state (e=enabled, d=disabled), virtual "
       "address, hit count, and associated command strings."},
      ...
      {"r_registers",
       ...
       "Description:\n"
       " - If called without arguments, displays all primary registers (eax/rax, ebx/rbx, eip/rip, efl/rflags, etc.) "
       "and the unassembled instruction at RIP.\n"
      ...
      {"uf_unassemble_function",
       ...
       "uf [AddressOrSymbol]",
       "Description:\n"
       " Unassembles instruction blocks belonging to the function containing the target address or symbol. "
       "Automatically resolves basic block branch paths rather than linear code."},
      {"peb_process_environment_block",
       ...
       "!peb [Address]",
       "Description:\n"
       " Shows basic environment parameters, loaded module list base, loader data pointers, session ID, and heap "
       "listings."},
